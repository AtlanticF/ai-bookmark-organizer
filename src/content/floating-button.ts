...
export function initFloatingButton() {
  try {
    ...
    chrome.storage.local.get("show_floating_button").then((result) => {
      if (result.show_floating_button === false) return;
      createButton();
    }).catch(() => createButton());

    chrome.storage.onChanged.addListener((changes, area) => {
      if (area !== "local" || !("show_floating_button" in changes)) return;
      const show = changes.show_floating_button.newValue;
      const existing = document.getElementById("ai-bookmark-fab-host");
...
<div class="fab-wrapper">
  ...
  <button class="hide-btn" title="Hide floating button">×</button>
</div>`;

  const btn = shadow.querySelector(".fab") as HTMLButtonElement;
  const hideBtn = shadow.querySelector(".hide-btn") as HTMLButtonElement;
  ...
  hideBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    chrome.storage.local.set({ show_floating_button: false }).catch(() => {});
    host.remove();
  });
